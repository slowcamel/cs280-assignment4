...
def discriminator_loss(logits_real, logits_fake):
    ...
    E_x = bce_loss(logits_real, true_labels)
    fake_labels = torch.zeros_like(logits_fake).type(dtype)
    E_z = bce_loss(logits_fake, fake_labels)
    loss = E_x + E_z
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    E_x = torch.mean((scores_real-1)**2)
    E_z = torch.mean((scores_fake)**2)
    loss = 0.5 * E_x + 0.5*E_z
